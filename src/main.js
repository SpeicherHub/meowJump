...
const assetBase = 'https://meowgame.tos-cn-beijing.volces.com/';
...
const scenes = [
  { id: 'basketball', start: 0, end: 50, name: '篮球乐园', praise: '小猫热身完成，弹跳手感已经在线！' },
  { id: 'candy', start: 50, end: 100, name: '糖果物语', praise: '你已经跳进甜甜云层，下一站会更高更闪亮！' },
  { id: 'forest', start: 100, end: 150, name: '森林秘境', praise: '穿过树梢的风都在给你鼓掌，继续向上冲！' },
  { id: 'chongqing', start: 150, end: 200, name: '魔幻重庆', praise: '你跳到了山城天际线，喵喵的弹跳力太巴适了！' },
  { id: 'space', start: 200, end: Infinity, name: '宇宙星河', praise: '已经飞向宇宙啦，这只小猫简直是星际跳跃冠军！' },
];
...
for (const scene of scenes) {
  manifest[`${scene.id}Background`] = assetUrl(`assets/scenes/${scene.id}-background.png`);
  manifest[`${scene.id}Platform`] = assetUrl(`assets/scenes/${scene.id}-platform-normal.png`);
  manifest[`${scene.id}PlatformFragile`] = assetUrl(`assets/scenes/${scene.id}-platform-fragile.png`);
}
...
const game = {
  ...
  player: { x: 0, y: 0, vx: 0, vy: 0, mode: 'normal', rocketTargetY: null },
};
...
function sceneForMeters(meters) {
...
function createRocketForPlatform(platform) {
  ...
  return { collected: false, bucket, offsetX: platform.width / 2 - config.rocketWidth / 2, offsetY: -config.rocketHeight + 6 };
}

function createPlatform(x, y, width = config.platformWidth, type = choosePlatformType(y)) {
  const platform = { x, y, width, height: config.platformHeight, type, rocket: null };
...
function createPlatformRow(y, margin = 16) {
  ...
  const rightX = randomBetween(leftX + config.platformWidth + config.doublePlatformMinGap, config.worldWidth - config.platformWidth - margin);
...
function resetGame() {
  ...
  game.platforms = [createPlatform(config.worldWidth / 2 - config.platformWidth / 2, worldHeight * config.startPlatformYRatio, config.platformWidth, 'normal')];
...
function endGame() {
  ...
  const result = sceneForMeters(game.score);
...
function getRocketRect(platform) {
  ...
  return { x: platform.x + platform.rocket.offsetX, y: platform.y + platform.rocket.offsetY, width: config.rocketWidth, height: config.rocketHeight };
}
...
function collectRocketIfTouched() {
  if (game.player.mode === 'rocket') return;
  const playerBounds = {
    x: game.player.x + config.playerWidth * 0.3,
    y: game.player.y + config.playerHeight * 0.16,
  ...
  for (const platform of game.platforms) {
    ...
    if (intersects(playerBounds, hitbox)) {
      platform.rocket.collected = true;
      game.player.mode = 'rocket';
      game.player.rocketTargetY = game.player.y - config.rocketBoostMeters * config.meterGridHeight;
      game.player.vy = -config.rocketBoostSpeed;
...
function updateCameraAndScore() {
  ...
  if (game.player.y < cameraTrigger) game.cameraY = game.player.y - worldHeight * config.cameraLiftRatio;
...
function update(dt) {
  ...
  player.vx = (input.right ? config.moveSpeed : 0) - (input.left ? config.moveSpeed : 0);
...
function drawBackground() {
  const scene = sceneForMeters(game.score);
  drawCoverImage(assets[`${scene.id}Background`], 0, 0, canvas.clientWidth, canvas.clientHeight);
}

function drawPlatform(platform) {
  const scene = sceneForMeters(metersFromY(platform.y));
  const img = assets[`${scene.id}${platform.type === 'fragile' ? 'PlatformFragile' : 'Platform'}`];
  ctx.drawImage(img, toScreenX(platform.x), toScreenY(platform.y - 12), platform.width * scale, config.platformDrawHeight * scale);
}
...
function drawRocketPowerup(platform) {
  ...
  ctx.drawImage(assets.rocketPowerup, toScreenX(rocketRect.x), toScreenY(rocketRect.y + floatOffset), config.rocketWidth * scale, config.rocketHeight * scale);
}

function drawPlayer() {
  const sprite = game.player.mode === 'rocket'
    ? assets.characterRocket
    : state === 'ready' || Math.abs(game.player.vy) < 80
      ? assets.characterIdle
      : game.player.vy < 0
        ? assets.characterJump
        : assets.characterFall;
  ctx.drawImage(sprite, toScreenX(game.player.x), toScreenY(game.player.y), config.playerWidth * scale, config.playerHeight * scale);
}
...
function draw() {
  ...
  game.platforms.forEach(drawRocketPowerup);
  drawPlayer();
}
...
function bindInput() {
  ...
  window.addEventListener('pointerup', () => { input.left = false; input.right = false; }, { passive: true });
  window.addEventListener('pointercancel', () => { input.left = false; input.right = false; }, { passive: true });
